...
def main(argv):
    ...
    if not os.path.exists(submission_path):
        raise ValueError(
            "Expected submission file %s, found files %s" % (submission_file_name, os.listdir(submission_dir)),
        )
    submission = {}
    rowsize = None
    with open(submission_path) as submission_file:
        ...
        for row in reader:
            if len(row) != 2 and len(row) != 6:
                raise ValueError("Wrong number of columns in line %d, expected 2 or 6." % count)
